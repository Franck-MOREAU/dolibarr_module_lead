...
class Lead extends CommonObject
{
	...
	function addRelance($dateRelance) {
		...
		$a->socid = $this->fk_soc;
		$a->datep = $dateRelance;
		$a->userownerid = $this->fk_user_resp;
	...
	function create($user, $notrigger = 0) {
		...
		if (! $error) {
			...
			$sql .= " '" . $this->db->idate($this->date_closure) . "',";
		...
		if (is_array($filter)) {
			foreach ( $filter as $key => $value ) {
				if (($key == 't.fk_c_status') || ($key == 't.rowid') || ($key == 'so.rowid') || ($key == 't.fk_c_type') || ($key == 't.fk_user_resp')
						|| ($key == 'leadextra.gamme') || ($key == 'leadextra.carroserie') || ($key == 'leadextra.genre') || ($key == 'leadextra.chaude')) {
					$sql .= ' AND ' . $key . ' = ' . $value;
